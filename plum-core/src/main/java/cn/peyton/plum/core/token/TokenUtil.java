...
public class TokenUtil<T> implements Serializable {
    ...
    public String sign(String userId, String username,long expireTime) {
        return JWT.create()
                ...
                .withClaim(TokenUtil.Property.USERID, userId)
                .withClaim(TokenUtil.Property.USERNAME, username)
                .withClaim(TokenUtil.Property.TIMESTAMP, System.currentTimeMillis())
    ...
    public boolean verify(String token) {
        try {
            ...
            log.info("Token 认证通过; 过期时间: {}", DateUtil.conventDate2Str(expiresAt));
            return true;
    ...
    public Map<String, String> parseToken(String token) {
        try {
            ...
            DecodedJWT jwt = JWT.require(Algorithm.HMAC256(tokenSecret)).withIssuer(getIssuer())
                    .build().verify(token);
            map.put(TokenUtil.Property.USERID, jwt.getClaim(TokenUtil.Property.USERID).asString());
            map.put(TokenUtil.Property.USERNAME, jwt.getClaim(TokenUtil.Property.USERNAME).asString());
            map.put(TokenUtil.Property.TIMESTAMP, jwt.getClaim(TokenUtil.Property.TIMESTAMP).asString());
            return map;
        } catch (Exception e) {
            return null;
        }
